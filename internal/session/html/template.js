    (function() {
      ...
      const injectedParams = document.querySelector('meta[name="pigo-url-params"]');
      ...
      function getSearchableText(entry, label) {
        ...
        switch (entry.type) {
          ...
          case 'active_tools_change':
            parts.push('tools', (entry.activeToolNames || []).join(' '));
            break;
        }
      ...
      function filterNodes(flatNodes, currentLeafId) {
        ...
        const filtered = flatNodes.filter(flatNode => {
          ...
          // Apply filter mode
          const isSettingsEntry = ['label', 'custom', 'model_change', 'thinking_level_change', 'active_tools_change'].includes(entry.type);
      ...
      function getTreeNodeDisplayHtml(entry, label) {
        ...
        switch (entry.type) {
          ...
          case 'active_tools_change':
            return labelHtml + `<span class="tree-muted">[tools: ${escapeHtml((entry.activeToolNames || []).join(', '))}]</span>`;
          default:
      ...
      function buildShareUrl(entryId) {
        ...
        const baseUrlMeta = document.querySelector('meta[name="pigo-share-base-url"]');
      ...
      const SIDEBAR_WIDTH_STORAGE_KEY = 'pigo-share:v1:sidebar-width';
